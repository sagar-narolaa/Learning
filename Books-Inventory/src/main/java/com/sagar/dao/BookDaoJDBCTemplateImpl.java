 ...
 * 
 * @Repository("booksJDBC") public class BookDaoJDBCTemplateImpl implements
 * BookDao {
 * 
 * @Autowired public JdbcTemplate template;
 * 
 * public void setTemplate(JdbcTemplate template) { this.template = template; }
 * 
 * @Override public List<Book> findAll() { return
 * template.execute(getQuery("get_all_books"), new
 * PreparedStatementCallback<List<Book>>() {
 * 
 * @Override public List<Book> doInPreparedStatement(PreparedStatement ps)
 * throws SQLException, DataAccessException {
 * 
 * ResultSet rs = ps.executeQuery();
 * 
 * Book book = null; List<Book> books = new ArrayList<>();
 * 
 * while (rs.next()) { book = new Book(); book.setName(rs.getString(2));
 * book.setId(rs.getInt(1)); book.setISBN(rs.getString(3));
 * book.setAuthor(rs.getString(4)); books.add(book); book = null; }
 * 
 * return books; }
 * 
 * }); }
 ...
 * template.execute(getQuery("insert_book"), new
 * PreparedStatementCallback<Boolean>() {
 * 
 * @Override public Boolean doInPreparedStatement(PreparedStatement ps) throws
 * SQLException, DataAccessException { ps.setString(1, book.getName());
 * ps.setString(2, book.getISBN()); ps.setString(3, book.getAuthor()); return
 * ps.execute(); }
 * 
 * });
 * 
 ...
 * template.execute(getQuery("update_book"), new
 * PreparedStatementCallback<Boolean>() {
 * 
 * @Override public Boolean doInPreparedStatement(PreparedStatement ps) throws
 * SQLException, DataAccessException { ps.setString(1, book.getName());
 * ps.setString(2, book.getISBN()); ps.setString(3, book.getAuthor());
 * ps.setInt(4, book.getId()); return ps.executeUpdate() > 0; }
 * 
 * });
 * 
 * }
 ...
 * template.execute(getQuery("delete_book_by_id"), new
 * PreparedStatementCallback<Boolean>() {
 * 
 * @Override public Boolean doInPreparedStatement(PreparedStatement ps) throws
 * SQLException, DataAccessException { ps.setInt(1, id); return ps.execute(); }
 * 
 * });
 * 
 * }
 * 
 * @Override public Book findById(int id) { return
 * template.execute(getQuery("select_book_by_id"), new
 * PreparedStatementCallback<Book>() {
 * 
 * @Override public Book doInPreparedStatement(PreparedStatement ps) throws
 * SQLException, DataAccessException { ps.setInt(1, id); ResultSet rs =
